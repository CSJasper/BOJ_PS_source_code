...
void print_arr(size_t**, matsize_t);
...
int main(void)
{
    ...
    if(matrix_size.row < 3 && matrix_size.column < 3) goto path2;

    while(1)
    {
        if(column_start == (matrix_size.column - 1) - 1)
        {
            row_start++;
            column_start = 0;
        }
        ...
        for(size_t current_row = row_start, current_column = column_start ; current_column < matrix_size.column ; current_row++)
        {
            if(counter == 3)
            {
                ...
                if(check.is_same[0] == false && check.is_same[1] == false && check.is_same[2] == false)
                {
                    ...
                    for(size_t tmp_column = current_column ; tmp_column < current_column + 3 ; tmp_column++)
                    {
